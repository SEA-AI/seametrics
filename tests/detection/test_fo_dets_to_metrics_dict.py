...
def fake_payload(gt_bboxes, pred_bboxes,from_nFoV:bool = False):
    ...
    for index, item in enumerate(pred_bboxes):
        ...
        #fo.Detection(label="SHIP",bounding_box=item,confidence=1)
    ...
    for index, item in enumerate(gt_bboxes):
        ...
        #var  = fo.Detection(label="SHIP",bounding_box=item,confidence=1,area=(item[2]*640)*(item[3]*512))
...
def test_det_metrics():
    ...
    assert int(processed_pred[0]['boxes'][0][0]) == int(pred_normalized[0]['bounding_box'][0] * img_res[1])
    assert int(processed_pred[0]['boxes'][0][1]) == int(pred_normalized[0]['bounding_box'][1] * img_res[0])
    assert int(processed_pred[0]['boxes'][0][2]) == int(pred_normalized[0]['bounding_box'][2] * img_res[1])
    assert int(processed_pred[0]['boxes'][0][3]) == int(pred_normalized[0]['bounding_box'][3] * img_res[0])

    assert len(processed_gt[0]['boxes'][0]) == 4
    assert len(processed_gt[0]['labels']) == 1

    assert int(processed_gt[0]['boxes'][0][0]) == int(gt_normalized[0]['bounding_box'][0] * img_res[1])
    assert int(processed_gt[0]['boxes'][0][1]) == int(gt_normalized[0]['bounding_box'][1] * img_res[0])
    assert int(processed_gt[0]['boxes'][0][2]) == int(gt_normalized[0]['bounding_box'][2] * img_res[1])
    assert int(processed_gt[0]['boxes'][0][3]) == int(gt_normalized[0]['bounding_box'][3] * img_res[0])    
    
    assert int(processed_gt[0]['area'][0]) == int(processed_gt[0]['boxes'][0][2] * processed_gt[0]['boxes'][0][3])
