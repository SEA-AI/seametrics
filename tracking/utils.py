...
def compute_metrics_by_sequence(
    ...
    sequence_results = {}
    sequence_names = get_relevant_fields(view, [gt_field, pred_field, 'sequence']).distinct("sequence")
    ...
    for sequence in sequence_results.keys():
        print(sequence)
        print(sequence_results[sequence][0])
        print(sequence_results[sequence][1])
        try:
...
def results_to_csv(metrics, sequence_list: list = None) -> None:
    # if sequence_list is not provided, compute for all sequences
    df = results_to_df(metrics, sequence_list)
