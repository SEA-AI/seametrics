"""HTML comparison report builder for seametrics metrics.

Generates an interactive HTML report with per-sequence bar charts and a
sortable comparison table.  Accepts any combination of tracking
(``TrackingMetrics``, ``HOTAMetrics``) and detection (``DetectionMetrics``)
DataFrames — each inner key becomes a tab in the report.
"""
...
def build_comparison_html(dfs: dict) -> str:
    ...
    Accepts any combination of tracking and detection metric DataFrames.
    Each key in the inner dict becomes a tab in the report.

    Parameters
    ----------
    dfs : dict
        Nested dict: ``{pred_field: {metric_name: df}}``.
        Common metric names: ``"TrackingMetrics"``, ``"HOTAMetrics"``,
        ``"Det (all)"``, ``"Det (small)"``, ``"Det (medium)"``,
        ``"Det (large)"``.  Any string is valid — it becomes the tab label.
        Each DataFrame must have a ``sequence`` column plus numeric metric
        columns produced by ``results_to_df`` (tracking) or
        ``det_metrics_to_df`` (detection).
